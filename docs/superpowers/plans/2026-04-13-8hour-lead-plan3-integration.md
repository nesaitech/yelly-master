# 8hour-lead Plan 3 — Mega-Skill Integration Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans. Steps use checkbox (`- [ ]`) syntax.

**Goal:** Wire the 4 8hour-lead modules from Plan 2 into a usable mega-skill `/8hour-lead`. Add the routing template, the 3 CLI tools (`sync`, `validate`, `stats`), the lib code that backs them, the setup script registration, and integration tests that prove the full chain (bootstrap → module → 8HOUR.md update → CLI verification) works.

**Architecture:** The mega-skill template loads `8HOUR.md` first, routes the user request to one of the 4 modules using a keyword table, and emits a permission pre-flight before doing any work. The CLI tools are thin wrappers around new functions in `lib/8hour-lead/` so the logic stays testable. The setup script picks up `8hour-lead` automatically once it is added to the `SKILLS` bash array.
...
**Spec reference:** spec sections §5.1, §5.3, §9 (Session continuation), §11 (CLI tools), §12 (file rows for `skills/8hour-lead/*`, `bin/8hour-lead-*`).

**Dependencies:** Plan 1 (lib/8hour-lead/*) and Plan 2 (modules/{adr,debt,estimate,risk}/) must be in place on this branch.
...
```
config/8hour-lead.yaml                   # mega-skill defaults block
skills/8hour-lead/SKILL.md.tmpl          # routing + permission pre-flight + bootstrap

lib/8hour-lead/validate.ts               # logic for 8hour-lead-validate
lib/8hour-lead/stats.ts                  # logic for 8hour-lead-stats
lib/8hour-lead/sync.ts                   # logic for 8hour-lead-sync

bin/8hour-lead-validate.ts               # CLI wrapper
bin/8hour-lead-stats.ts                  # CLI wrapper
bin/8hour-lead-sync.ts                   # CLI wrapper

test/unit/8hour-lead/validate.test.ts
test/unit/8hour-lead/stats.test.ts
test/unit/8hour-lead/sync.test.ts
test/integration/8hour-lead-routing.test.ts
```
...
package.json              # add 8hour-lead-sync, 8hour-lead-validate, 8hour-lead-stats npm scripts
setup                     # add "8hour-lead" to SKILLS array on line 147
```
...
// lib/8hour-lead/sync.ts
export interface SyncResult {
  eightHourMdRewritten: boolean;
  sectionsTouched: string[];
}
// Re-render 8HOUR.md sections from durable artifacts (docs/8hour/adr/*.md, docs/8hour/risks/active.md, etc.)
export function syncEightHourMd(projectDir: string, eightHourLeadVersion: string): SyncResult;
```

---

## Task 1: config — defaults and 8hour-lead.yaml

**Files:**
- Modify: `config/defaults.yaml`
- Create: `config/8hour-lead.yaml`

- [ ] **Step 1: Add 8hour-lead module defaults to `config/defaults.yaml`**
...
- [ ] **Step 2: Create `config/8hour-lead.yaml`**

```yaml
# config/8hour-lead.yaml
# Mega-skill defaults for /8hour-lead. User overrides go to ~/.8hour-master/config.yaml
# under a 8hour_lead: block.

8hour_lead:
  bootstrap:
    auto_inject_claude_md: true        # add @8HOUR.md to host CLAUDE.md on first run
    create_gitignore_entry: true        # add .8hour/ to project .gitignore
  8hour_md:
    max_lines: 400
    decision_log_max_entries: 10
  artifacts_dir: docs/8hour
  state_dir: .8hour
  history_file: .8hour/history/estimates.jsonl
```
...
```bash
git add config/defaults.yaml config/8hour-lead.yaml
git commit -m "feat(8hour-lead): add defaults + 8hour-lead.yaml mega-skill config"
```

---

## Task 2: skills/8hour-lead/SKILL.md.tmpl

**Files:**
- Create: `skills/8hour-lead/SKILL.md.tmpl`
...
description: |
  ...
  Invoke with /8hour-lead or when user asks to estimate, decide
  architecture, track risk, or manage technical debt.
---

{{PREAMBLE}}

# 8hour-lead — Tech Lead Strategic Core

You are an expert tech lead. When this skill is invoked, you operate from a *project state* anchored in the project's `8HOUR.md` file. Read `8HOUR.md` BEFORE doing anything else.

## Session Bootstrap (every invocation)

1. **Read `8HOUR.md`** at the project root. Load full content into context.
2. If `8HOUR.md` does **not** exist → trigger the bootstrap flow (see below).
3. Parse the frontmatter. If `8hour_version` is missing or older than the current schema, run the migration before proceeding.
...
When `8HOUR.md` is missing:

1. Detect the stack via `lib/8hour-lead/bootstrap.ts:gatherContext(projectDir)`.
...
3. Render the initial `8HOUR.md` via `renderEightHourMdTemplate` from `templates/8HOUR.md.tmpl`, passing the gathered context + answers.
4. Write `8HOUR.md` to the project root.
5. Scaffold `docs/8hour/{adr,estimates,risks,decisions}/` with `.gitkeep` placeholders.
6. Append `.8hour/` to `.gitignore` (if not already present).
7. Commit with message `chore(8hour): initialize project tech lead context`.
...
```
🎯 8hour-lead: Preparing for [task description]
...
## 8HOUR.md Update Discipline

After completing any module workflow, you MUST update `8HOUR.md` using `lib/8hour-lead/8hour-md-updater.ts`:

1. `replaceSection` for sections the module owns (e.g., adr → architecture-decisions)
2. `appendToSection` for the decision-log entry
3. `stampFrontmatter` with `updated_by: "/8hour-lead <module>"` and the current `8hour_lead_version`
4. Run `{{BIN_ROOT}}/8hour-lead-validate` to check the result is well-formed

If `8HOUR.md` exceeds the configured `max_lines` (default 400), rotate the oldest half of the Decision Log entries into `docs/8hour/decisions/archive.md`.
...
```bash
{{BIN_ROOT}}/8hour-config get 8hour_lead.8hour_md.max_lines
{{BIN_ROOT}}/8hour-config get adr.format
```

## CLI Tools

- `{{BIN_ROOT}}/8hour-lead-sync` — regenerate `8HOUR.md` from sources after manual edits
- `{{BIN_ROOT}}/8hour-lead-validate` — check `8HOUR.md` schema, ADR filenames, orphan artifacts
- `{{BIN_ROOT}}/8hour-lead-stats` — show ADR count, risk closure rate, estimate calibration
```

- [ ] **Step 2: Type-check is N/A; verify file exists**

Run: `cat skills/8hour-lead/SKILL.md.tmpl | head -10`
Expected: shows the frontmatter `name: 8hour-lead`.
...
```bash
git add skills/8hour-lead/SKILL.md.tmpl
git commit -m "feat(8hour-lead): add /8hour-lead mega-skill template"
```

---

## Task 3: lib/8hour-lead/validate.ts and bin/8hour-lead-validate.ts

**Files:**
- Create: `lib/8hour-lead/validate.ts`
- Create: `bin/8hour-lead-validate.ts`
- Create: `test/unit/8hour-lead/validate.test.ts`
...
`,
  ...
  it("flags missing 8hour_version", () => {
    ...
    mkdirSync(dir, { recursive: true });
    writeFileSync(join(dir, "8HOUR.md"), "---\nlast_updated: 2026-04-13T00:00:00Z\n---\n# 8HOUR\n");
    const result = validateProjectDir(dir);
    expect(result.ok).toBe(false);
...
Run: `npx vitest run test/unit/8hour-lead/validate.test.ts`
Expected: FAIL — module not found.

- [ ] **Step 3: Create `lib/8hour-lead/validate.ts`**
...
- [ ] **Step 5: Create `bin/8hour-lead-validate.ts`**
...
```bash
git add lib/8hour-lead/validate.ts bin/8hour-lead-validate.ts test/unit/8hour-lead/validate.test.ts
git commit -m "feat(8hour-lead): add validate lib + CLI"
```

---

## Task 4: lib/8hour-lead/stats.ts and bin/8hour-lead-stats.ts

**Files:**
- Create: `lib/8hour-lead/stats.ts`
- Create: `bin/8hour-lead-stats.ts`
- Create: `test/unit/8hour-lead/stats.test.ts`
...
describe("gatherStats", () => {
  ...
  it("reads 8HOUR.md line count and last_updated", () => {
    ...
    expect(stats.eightHourMdLastUpdated).toBe("2026-04-13T00:00:00Z");
    rmSync(tmp, { recursive: true, force: true });
  });
});
```
...
Run: `npx vitest run test/unit/8hour-lead/stats.test.ts`
Expected: FAIL — module not found.

- [ ] **Step 3: Create `lib/8hour-lead/stats.ts`**
...
- [ ] **Step 5: Create `bin/8hour-lead-stats.ts`**
...
```bash
git add lib/8hour-lead/stats.ts bin/8hour-lead-stats.ts test/unit/8hour-lead/stats.test.ts
git commit -m "feat(8hour-lead): add stats lib + CLI"
```

---

## Task 5: lib/8hour-lead/sync.ts and bin/8hour-lead-sync.ts

The sync command rebuilds the dynamic sections of `8HOUR.md` from the durable artifacts in `docs/8hour/`. It does NOT touch Pinned Notes or Decision Log (those are append-only or human-curated).

**Files:**
- Create: `lib/8hour-lead/sync.ts`
- Create: `bin/8hour-lead-sync.ts`
- Create: `test/unit/8hour-lead/sync.test.ts`
...
Run: `npx vitest run test/unit/8hour-lead/sync.test.ts`
Expected: FAIL — module not found.

- [ ] **Step 3: Create `lib/8hour-lead/sync.ts`**
...
- [ ] **Step 5: Create `bin/8hour-lead-sync.ts`**
...
console.log(`✅ 8HOUR.md synced (${result.sectionsTouched.length} section(s) touched)`);
...
```bash
git add lib/8hour-lead/sync.ts bin/8hour-lead-sync.ts test/unit/8hour-lead/sync.test.ts
git commit -m "feat(8hour-lead): add sync lib + CLI"
```
...
Insert three new entries in the `"scripts"` block (after the existing `8hour-repo-mode` line):

```json
"8hour-lead-sync": "node --loader ts-node/esm bin/8hour-lead-sync.ts",
"8hour-lead-validate": "node --loader ts-node/esm bin/8hour-lead-validate.ts",
"8hour-lead-stats": "node --loader ts-node/esm bin/8hour-lead-stats.ts",
```
...
Run:
```bash
npm run 8hour-lead-stats -- /tmp
npm run 8hour-lead-validate -- /tmp || true
```

Expected: both commands execute without crashing. `8hour-lead-stats` prints zero counts. `8hour-lead-validate` reports "8HOUR.md not found".

- [ ] **Step 3: Add `8hour-lead` to `setup` SKILLS array**

Edit `setup` line 147. Change:

```bash
SKILLS=("8hour-code" "8hour-ops" "8hour-quality" "8hour-team")
```

to:

```bash
SKILLS=("8hour-code" "8hour-ops" "8hour-quality" "8hour-team" "8hour-lead")
```
...
git commit -m "feat(8hour-lead): register CLI tools and 8hour-lead skill in setup"
```
...
**Files:**
- Create: `test/integration/8hour-lead-routing.test.ts`

This test asserts that the public API (lib/8hour-lead/*) composes correctly to produce a usable 8HOUR.md from a fixture project.
...
describe("8hour-lead end-to-end routing", () => {
  ...
  it("bootstrap → ADR → risk → sync → validate → stats", () => {
    ...
    // 2. Bootstrap: render 8HOUR.md from the real template
    ...
    // 3. Validate the freshly bootstrapped 8HOUR.md
    let result = validateProjectDir(project);
    expect(result.ok).toBe(true);

    // 4. Add an ADR to docs/8hour/adr/
    ...
    // 5. Add a risk to docs/8hour/risks/active.md
    ...
    // 9. 8HOUR.md content reflects sync output
...
Run: `npx vitest run test/integration/8hour-lead-routing.test.ts`
Expected: 2/2 PASS.
...
```bash
git add test/integration/8hour-lead-routing.test.ts
git commit -m "test(8hour-lead): add end-to-end routing integration test"
```
...
Run:
```bash
ls skills/8hour-lead/
cat skills/8hour-lead/SKILL.md.tmpl | head -10
```

Expected: template file exists with the `name: 8hour-lead` frontmatter.
...
Run:
```bash
ls config/8hour-lead.yaml skills/8hour-lead/SKILL.md.tmpl
ls bin/8hour-lead-sync.ts bin/8hour-lead-validate.ts bin/8hour-lead-stats.ts
ls lib/8hour-lead/sync.ts lib/8hour-lead/validate.ts lib/8hour-lead/stats.ts
ls test/integration/8hour-lead-routing.test.ts
```
...
Expected: clean tree, recent commits show 6 task commits matching `feat(8hour-lead):` / `test(8hour-lead):`.

---

## Done Criteria (Plan 3)

- [x] `skills/8hour-lead/SKILL.md.tmpl` exists with routing + permission pre-flight + bootstrap section
- [x] `config/8hour-lead.yaml` defines mega-skill defaults; `config/defaults.yaml` includes per-module 8hour-lead defaults
...
- [x] `setup` script `SKILLS` array includes `8hour-lead`
...
- CHANGELOG entry
- `docs/8hour-lead/OVERVIEW.md` and `docs/8hour-lead/EIGHT_HOUR_MD_SCHEMA.md`
- `migrations/v0.2.0-8hour-lead.ts`
