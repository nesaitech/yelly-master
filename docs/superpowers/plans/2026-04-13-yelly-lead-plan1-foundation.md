# yelly-lead Plan 1 — Foundation Library Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship the foundation library `lib/yelly-lead/*` and template files that all 4 yelly-lead modules depend on — parsing frontmatter, detecting issue trackers, safely updating the project `YELLY.md` context file, and producing initial content from templates.

**Architecture:** Pure-function TypeScript library with no AI coupling — the AI orchestration layer (modules + SKILL.md.tmpl) will call these functions. All filesystem operations are explicit; templates use `{{VAR}}` placeholder substitution (same convention as existing `SKILL.md.tmpl`); `YELLY.md` sections are delimited by HTML comment markers so the updater can locate and rewrite them idempotently.
...
**Spec reference:** `docs/superpowers/specs/2026-04-13-yelly-lead-mvp-design.md` sections 5 (Architecture), 7 (YELLY.md Schema), 8 (Bootstrap Flow), 12 (File Inventory lib rows).
...
| Path | Responsibility |
|------|---------------|
| `lib/yelly-lead/frontmatter.ts` | Parse and serialize YAML frontmatter blocks on markdown files |
| `lib/yelly-lead/tracker-detect.ts` | Detect available issue tracker CLIs (gh, glab, jira, linear) in current project |
| `lib/yelly-lead/yelly-md-updater.ts` | Replace section content inside `YELLY.md` using HTML-comment markers, preserving non-managed regions |
| `lib/yelly-lead/bootstrap.ts` | Pure functions for first-run context gathering (stack detection, git stats) and initial `YELLY.md` generation from template |
| `lib/yelly-lead/index.ts` | Barrel export — re-exports all public types and functions |
| `templates/YELLY.md.tmpl` | Root project state template with section markers |
...
| `test/unit/yelly-lead/frontmatter.test.ts` | Unit tests for frontmatter parser |
| `test/unit/yelly-lead/tracker-detect.test.ts` | Unit tests for tracker detection |
| `test/unit/yelly-lead/yelly-md-updater.test.ts` | Unit tests for section replacement and governance |
| `test/unit/yelly-lead/bootstrap.test.ts` | Unit tests for stack detection and generation |
| `test/static/yelly-lead-templates.test.ts` | Static tests — each template file contains required markers and placeholders |
...
export interface UpdateMeta { updated_by: string; yelly_lead_version: string }
...
**Files:**
- Create: `lib/yelly-lead/frontmatter.ts`
- Create: `test/unit/yelly-lead/frontmatter.test.ts`
...
Run: `npx vitest run test/unit/yelly-lead/frontmatter.test.ts`
Expected: FAIL — "Cannot find module '../../../lib/yelly-lead/frontmatter.js'"
...
```bash
git add lib/yelly-lead/frontmatter.ts test/unit/yelly-lead/frontmatter.test.ts
git commit -m "feat(yelly-lead): add frontmatter parser/serializer lib"
```
...
**Files:**
- Create: `lib/yelly-lead/tracker-detect.ts`
- Create: `test/unit/yelly-lead/tracker-detect.test.ts`
...
import { join } from "path";
import { tmpdir } from "os";
import { detectTracker } from "../../../lib/yelly-lead/tracker-detect.js";

describe("detectTracker", () => {
  const tmp = join(tmpdir(), "yelly-tracker-test-" + Date.now());
...
Run: `npx vitest run test/unit/yelly-lead/tracker-detect.test.ts`
...
```bash
git add lib/yelly-lead/tracker-detect.ts test/unit/yelly-lead/tracker-detect.test.ts
git commit -m "feat(yelly-lead): add tracker detection (github/gitlab/jira/linear)"
```

---

## Task 3: YELLY.md template with section markers

**Files:**
- Create: `templates/YELLY.md.tmpl`
- Create: `test/static/yelly-lead-templates.test.ts`
...
---
yelly_version: 1
yelly_lead_version: {{YELLY_LEAD_VERSION}}
...
> Regenerate or refresh with `yelly-lead-sync`.
...
_None yet. Start a task with `/yelly-lead estimate <feature>` or `/yelly-lead adr <decision>`._
<!-- /yelly-lead: active-work -->
...
- {{LAST_UPDATED}} — Project initialized via /yelly-lead bootstrap
<!-- /yelly-lead: decision-log -->
...
Run: `npx vitest run test/static/yelly-lead-templates.test.ts`
Expected: PASS — template file exists and satisfies all markers + placeholders

- [ ] **Step 4: Commit**

```bash
git add templates/YELLY.md.tmpl test/static/yelly-lead-templates.test.ts
git commit -m "feat(yelly-lead): add YELLY.md template with section markers"
```
...
- Modify: `test/static/yelly-lead-templates.test.ts` (extend)
...
Append this block to `test/static/yelly-lead-templates.test.ts` at the end of the file:
...
```bash
git add templates/adr.md.tmpl templates/estimate.md.tmpl templates/risk.md.tmpl test/static/yelly-lead-templates.test.ts
git commit -m "feat(yelly-lead): add adr, estimate, risk templates"
```

---

## Task 5: yelly-md-updater.ts — replaceSection and appendToSection

**Files:**
- Create: `lib/yelly-lead/yelly-md-updater.ts`
- Create: `test/unit/yelly-lead/yelly-md-updater.test.ts`
...
Run: `npx vitest run test/unit/yelly-lead/yelly-md-updater.test.ts`
Expected: FAIL — module not found
...
```bash
git add lib/yelly-lead/yelly-md-updater.ts test/unit/yelly-lead/yelly-md-updater.test.ts
git commit -m "feat(yelly-lead): add YELLY.md section updater with replace/append"
```
...
**Files:**
- Create: `lib/yelly-lead/bootstrap.ts`
- Create: `test/unit/yelly-lead/bootstrap.test.ts`
...
describe("gatherContext", () => {
  const tmp = join(tmpdir(), "yelly-boot-test-" + Date.now());
...
describe("renderYellyMdTemplate", () => {
  const tmp = join(tmpdir(), "yelly-render-test-" + Date.now());

  it("substitutes placeholders", () => {
    mkdirSync(tmp, { recursive: true });
    const tplPath = join(tmp, "YELLY.md.tmpl");
    writeFileSync(
      tplPath,
      [
        "---",
    ...
    const output = renderYellyMdTemplate(
      tplPath,
      {
        projectDir: tmp,
        projectName: "demo",
    ...
    expect(output).toContain("yelly_lead_version: 0.2.0");
...
Run: `npx vitest run test/unit/yelly-lead/bootstrap.test.ts`
Expected: FAIL — module not found
...
```bash
git add lib/yelly-lead/bootstrap.ts test/unit/yelly-lead/bootstrap.test.ts
git commit -m "feat(yelly-lead): add bootstrap context gathering + template rendering"
```
...
**Files:**
- Create: `lib/yelly-lead/index.ts`
...
Run: `npm test`
Expected: all existing tests continue passing + all new yelly-lead tests green

- [ ] **Step 4: Commit**

```bash
git add lib/yelly-lead/index.ts
git commit -m "feat(yelly-lead): add barrel export for lib/yelly-lead"
```

---

## Task 8: Wire yelly-lead tests into npm scripts
...
Run: `npx vitest run test/unit/yelly-lead/ test/static/yelly-lead-templates.test.ts`
Expected: all yelly-lead tests run green — no script changes required (vitest globs `test/unit/**` and `test/static/**` by default)
...
"test:static": "vitest run test/static/",
"test:unit": "vitest run test/unit/",
"test:yelly-lead": "vitest run test/unit/yelly-lead/ test/static/yelly-lead-templates.test.ts",
```

- [ ] **Step 4: Run the new script**

Run: `npm run test:yelly-lead`
Expected: all yelly-lead tests pass, no unrelated tests run

- [ ] **Step 5: Commit**

```bash
git add package.json
git commit -m "chore(yelly-lead): add test:yelly-lead npm script"
```
...
Run: `ls -la lib/yelly-lead/ templates/ test/unit/yelly-lead/ test/static/yelly-lead-templates.test.ts`
Expected output includes:
- `lib/yelly-lead/frontmatter.ts`
- `lib/yelly-lead/tracker-detect.ts`
- `lib/yelly-lead/yelly-md-updater.ts`
- `lib/yelly-lead/bootstrap.ts`
- `lib/yelly-lead/index.ts`
- `templates/YELLY.md.tmpl`
- `templates/adr.md.tmpl`
- `templates/estimate.md.tmpl`
- `templates/risk.md.tmpl`
- `test/unit/yelly-lead/frontmatter.test.ts`
- `test/unit/yelly-lead/tracker-detect.test.ts`
- `test/unit/yelly-lead/yelly-md-updater.test.ts`
- `test/unit/yelly-lead/bootstrap.test.ts`
- `test/static/yelly-lead-templates.test.ts`
...
Expected: a sequence of `feat(yelly-lead):` commits matching the task order
...
- [x] `package.json` has a `test:yelly-lead` convenience script

## Not In Plan 1 (will come in later plans)

- Module content (`modules/estimate`, `modules/risk`, `modules/adr`, `modules/debt`) → Plan 2
- `/yelly-lead` SKILL.md.tmpl, routing, permission pre-flight → Plan 3
- `bin/yelly-lead-sync`, `yelly-lead-validate`, `yelly-lead-stats` CLI tools → Plan 3
- Setup script integration + CLAUDE.md injection → Plan 3
- README, CHANGELOG, OVERVIEW, migration script, version bump → Plan 4

## Self-Review Notes

- **Spec coverage:** Library rows from spec §12 fully covered (`frontmatter.ts`, `yelly-md-updater.ts`, `bootstrap.ts`, `tracker-detect.ts`) + all 4 templates + initial static + unit tests.
