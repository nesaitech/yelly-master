---
title: 8hour-lead MVP — Tech Lead Mega-Skill
date: 2026-04-13
author: /8hour-lead brainstorming session
status: approved
phase: 0 (MVP)
...
# 8hour-lead MVP — Tech Lead Mega-Skill

## 1. Summary

Add a fifth mega-skill `/8hour-lead` to 8hour-master, providing tech lead capabilities
...
The MVP introduces a project-level context file (`8HOUR.md` at root) that captures
project state and prevents context loss across AI sessions — inspired by superpowers'
spec/plan persistence pattern.

## 2. Motivation

The existing 4 mega-skills cover engineering (8hour-code), operations (8hour-ops),
quality (8hour-quality), and team (8hour-team). A tech lead needs **strategic**
...
using 8hour-master must invent their own process each session, and lose context
between sessions.

## 3. Goals

- Ship 4 new modules (`estimate`, `risk`, `adr`, `debt`) as a new mega-skill `/8hour-lead`
- Introduce `8HOUR.md` (project root) as a durable, git-tracked project context file
- Introduce `docs/8hour/` as a directory for append-only artifacts
- Enable session continuity — a new AI session reads `8HOUR.md` at start
...
```
<project-root>/
├── 8HOUR.md                   # Project state snapshot (always current, git-tracked, ≤400 lines)
└── docs/8hour/
    ├── adr/
...
`.8hour/` is git-ignored by the bootstrap flow (adds to `.gitignore`).
...
└── hooks/
    ├── pre.sh                 # Load 8HOUR.md, detect tracker
    └── post.sh                # Update 8HOUR.md, append artifact
```
...
6. Historical calibration — load `.8hour/history/estimates.jsonl`, compute bias
7. Output — estimate document with range, confidence, assumptions, risks
8. Closure — when feature ships, user runs `/8hour-lead estimate --close <id>` to record actual
...
permissions:
  ...
  file_access:
    read: ["**/*"]
    write: ["docs/8hour/estimates/**/*", "8HOUR.md", ".8hour/history/**/*"]
```
...
4. Track — `docs/8hour/risks/active.md`, top 5 surface in `8HOUR.md`
5. Integrate:
...
6. Archive — when closed, move to `docs/8hour/risks/archive/YYYY-MM.md` with outcome
...
permissions:
  ...
  file_access:
    read: ["**/*"]
    write: ["docs/8hour/risks/**/*", "8HOUR.md"]
```
...
3. Naming — `docs/8hour/adr/NNNN-kebab-title.md`, NNNN sequential zero-padded
...
6. Surface — latest 5 ADRs appear in `8HOUR.md` Architecture Decisions section
...
**Methodology (guide.md outline):**
1. Detect tracker — check for `.github/`, Jira CLI, Linear CLI, GitLab; cache in `.8hour/cache/issue-tracker.json`
2. Scan sources — `8hour-quality/health` report, `TODO` / `FIXME` / `HACK` comments, complexity hotspots
...
5. Export strategy:
   ...
   - If no tracker → write `docs/8hour/debt/register.md`
6. Surface — top 5 debt items (by priority × cost-to-fix ratio) in `8HOUR.md`
...
permissions:
  ...
  file_access:
    read: ["**/*"]
    write: ["docs/8hour/debt/**/*", "8HOUR.md"]
```
...
> Auto-generated by /8hour-lead. Do not edit manually except for pinned notes.
> Regenerate with `8hour-lead-sync`.
...
**Version field maintenance:** `8hour_lead_version` in frontmatter is sourced from the root `VERSION` file at each update. Migration script (`migrations/v0.X.Y-*.ts`) runs automatically when the parsed `8hour_lead_version` is behind the installed version.

## 8. Bootstrap Flow (first-run)

When `/8hour-lead` runs and `8HOUR.md` does not exist:
...
5. **Generate** `8HOUR.md` from `templates/8HOUR.md.tmpl` with detected + answered values
6. **Scaffold** `docs/8hour/{adr,estimates,risks,decisions}/` with `.gitkeep` placeholders
7. **Add** `.8hour/` to `.gitignore` if not present
8. **Commit** — message: `chore(8hour): initialize project tech lead context`
9. **Report** — show user what was created and how to invoke modules

Bootstrap lives in `lib/8hour-lead/bootstrap.ts`, called from SKILL.md routing when `8HOUR.md` missing.

## 9. Session Continuation Protocol

Every `/8hour-lead` invocation follows this order:

1. **Check `8HOUR.md`** — if missing → trigger bootstrap
2. **Read `8HOUR.md`** — load full content into context
3. **Parse frontmatter** — verify `8hour_version` is current, else run migration
...
7. **Update** — post-hook writes back to `8HOUR.md` + appends to `docs/8hour/*`
8. **Validate** — post-hook checks `8HOUR.md` size ≤400 lines. If over: rotate oldest half of `Decision Log` entries into `docs/8hour/decisions/archive.md`; archive resolved items from `Top Risks` / `Tech Debt` into respective archive files.

**Cross-mega-skill loading path:** Since `8hour-lead` modules need to read guides from other mega-skills, use the shared `{{MODULES_ROOT}}` template variable (already populated by setup script). All modules live under one `modules/` tree regardless of which mega-skill exposes them, so paths are resolved as `{{MODULES_ROOT}}/<module-name>/guide.md`.

**CLAUDE.md injection — host compatibility:**
- Claude Code: append `@8HOUR.md` line to `~/.claude/CLAUDE.md` if exists, else prompt user to create one
- Codex / Cursor / Windsurf / Kiro: append to respective instructions file per host config
- If the target instructions file does not exist: create a minimal one referencing only `8HOUR.md`
- All injection operations are idempotent (check existing line before appending)
- Setup script records injections in `~/.8hour-master/injections.log` so uninstall can reverse them

## 10. Cross-Mega-Skill Integration

`8hour-lead` modules **consume** data from existing mega-skills via cross-module loading:

| From | Data | Used by | Purpose |
|------|------|---------|---------|
| `8hour-quality/health` | complexity, churn, coverage | `debt` | Candidate detection |
| `8hour-code/review` | PR history, diff stats | `estimate`, `risk` | Velocity, hotspots |
| `8hour-ops/deploy` | release log, rollback rate | `risk` | Production stability signal |
| `8hour-team/retro` | action items, patterns | `debt`, 8HOUR.md open questions | Surface recurring issues |
...
- **`8hour-lead-sync`** — regenerate `8HOUR.md` from `docs/8hour/` sources (useful after manual edits or drift)
- **`8hour-lead-validate`** — check `8HOUR.md` frontmatter, ADR filenames, orphan files in `docs/8hour/`
- **`8hour-lead-stats`** — compute estimate accuracy, risk closure rate, ADR count, debt burn-down

All three are thin TypeScript wrappers around `lib/8hour-lead/*` modules. Exposed via `package.json` scripts and `bin/` symlinks.
...
| `skills/8hour-lead/SKILL.md.tmpl` | mega-skill routing | 110 |
| `skills/8hour-lead/SKILL.md` | generated | - |
| `config/8hour-lead.yaml` | mega-skill defaults | 30 |
| `templates/8HOUR.md.tmpl` | bootstrap template | 80 |
...
| `lib/8hour-lead/bootstrap.ts` | first-run logic | 200 |
| `lib/8hour-lead/8hour-md-updater.ts` | safe update + merge | 180 |
| `lib/8hour-lead/tracker-detect.ts` | issue tracker detection | 90 |
| `lib/8hour-lead/frontmatter.ts` | parse/write frontmatter | 70 |
| `bin/8hour-lead-sync` | CLI | 40 |
| `bin/8hour-lead-validate` | CLI | 50 |
| `bin/8hour-lead-stats` | CLI | 60 |
| `test/static/8hour-lead.test.ts` | structure tests | 80 |
| `test/unit/8hour-md-updater.test.ts` | unit tests | 120 |
| `test/unit/bootstrap.test.ts` | unit tests | 100 |
| `test/unit/frontmatter.test.ts` | unit tests | 60 |
| `test/integration/8hour-lead-routing.test.ts` | integration | 90 |
| `test/fixtures/8hour-lead-project/` | test fixture | - |
| `migrations/v0.2.0-8hour-lead.ts` | migration script | 50 |
| `docs/8hour-lead/OVERVIEW.md` | user guide | 150 |
| `docs/8hour-lead/EIGHT_HOUR_MD_SCHEMA.md` | schema spec | 100 |
...
| `CHANGELOG` | Add `feat(8hour-lead): add tech lead mega-skill (MVP)` |
| `setup` | Register `/8hour-lead` skill, add `@8HOUR.md` to host CLAUDE.md |
| `scripts/gen-skills.js` | Support 5 mega-skills instead of 4 |
| `config/defaults.yaml` | Add 8hour-lead defaults block |
...
1. **Foundation** — lib/8hour-lead: frontmatter, bootstrap, updater, tracker-detect + unit tests
2. **Templates** — 8HOUR.md, adr, estimate, risk templates
...
7. **Mega-skill: 8hour-lead SKILL.md.tmpl** — routing, permission pre-flight, bootstrap trigger
...
3. `./setup` registers `/8hour-lead` for Claude Code + Codex + Cursor + Windsurf + Kiro
4. First-run on a fresh test fixture generates `8HOUR.md` + `docs/8hour/` skeleton + `.gitignore` entry
5. `/8hour-lead estimate <feature>` produces a document in `docs/8hour/estimates/` and updates `8HOUR.md` Active Work section
6. `/8hour-lead adr "use Postgres"` creates `docs/8hour/adr/0001-use-postgres.md` in Nygard format and updates `8HOUR.md` Architecture Decisions section
7. `/8hour-lead risk` updates `docs/8hour/risks/active.md` and `8HOUR.md` Top Risks section
8. `/8hour-lead debt` either creates issues in detected tracker or writes `docs/8hour/debt/register.md`, and updates `8HOUR.md` Tech Debt section
9. A new AI session (fresh Claude/Codex instance) automatically reads `8HOUR.md` via CLAUDE.md injection and demonstrates retained context
10. `8HOUR.md` stays under 400 lines after 20 simulated updates (governance test)
11. `8hour-lead-sync`, `8hour-lead-validate`, `8hour-lead-stats` CLI tools execute without error on fixture
12. `README.md`, `CHANGELOG`, `docs/8hour-lead/OVERVIEW.md`, `docs/8hour-lead/EIGHT_HOUR_MD_SCHEMA.md` all updated
...
- **Multi-tech-lead coordination** — 8HOUR.md merge conflicts in teams >2 leads. Defer; observe real usage first.
