...
description: |
  ...
  and containerization. Invoke with /8hour-ops or when user asks about deploy,
...
# 8hour-ops — Operations & Infrastructure
...
```
🔧 8hour-ops: Preparing for [task description]
...
- Common cross-module flows:
  ...
  - env → security (secrets audit) — cross mega-skill, load from ~/.claude/skills/8hour-master/modules/security/guide.md
...
Example: `~/.claude/skills/8hour-master/bin/8hour-config get deploy.platform`
